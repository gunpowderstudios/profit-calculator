(() => {
  ...
  const inputs = [
    ...
    'distDiscount','distQty','palletCost','unitsPerPallet','distReturns','distOther',
  ...
  function channelCalculation() {
    ...
    if (channel === 'distributor') {
      ...
      const unitsPerPallet = Math.max(1, num('unitsPerPallet', 1));
      const deliveryPerUnit = num('palletCost') / unitsPerPallet;
      revenueExVat = rrpVat.exVat * (1 - discount);
      vat = rrpVat.vat;
      channelCosts = {
        'UK pallet delivery / unit': deliveryPerUnit,
    ...
    return { channel, label, grossPrice, revenueExVat, vat, productParts: base.parts, productTotal: base.total, channelCosts, channelTotal, contribution, margin, quantity, variableRateGross, fixedChannel };
  }
  ...
  function targetDeal(calc) {
    ...
    if (calc.channel === 'distributor') {
      ...
      const returns = num('distReturns', 0, 100) / 100;
      const delivery = num('palletCost') / Math.max(1, num('unitsPerPallet', 1));
      ...
      return { label: 'Maximum distributor discount', value: pct(Math.min(100, maxDiscount)), note: `Highest discount that still leaves about a ${pct(target * 100)} contribution margin.` };
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('gunpowder-profit-calculator-v1', JSON.stringify(state));
    } catch (_) { }
  }

  function restoreState() {
    try {
      const raw = localStorage.getItem('gunpowder-profit-calculator-v1');
      if (!raw) return false;
